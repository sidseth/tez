...
import org.apache.tez.daemon.impl.TezDaemonProtocolClientImpl;
...
public class DaemonContainerLauncher extends AbstractService implements ContainerLauncher {
  ...
  private final Map<String, TezDaemonProtocolBlockingPB> proxyMap;
  ...
  public DaemonContainerLauncher(AppContext appContext, Configuration conf,
    ...
    this.servicePort = conf.getInt(TezDaemonConfiguration.TEZ_DAEMON_RPC_PORT,
    ...
    this.tal = tal;
    this.proxyMap = new HashMap<String, TezDaemonProtocolBlockingPB>();
  }
  ...
  private synchronized TezDaemonProtocolBlockingPB getProxy(String hostname) {
    TezDaemonProtocolBlockingPB proxy = proxyMap.get(hostname);
    if (proxy == null) {
      proxy = new TezDaemonProtocolClientImpl(getConfig(), hostname, servicePort);
  ...
  private static class SubmitCallable implements Callable<Void> {
    ...
    private final TezDaemonProtocolBlockingPB daemonProxy;

    private SubmitCallable(TezDaemonProtocolBlockingPB daemonProxy,
